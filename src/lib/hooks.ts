...
/* ------------------------------------------------------------------ */
/* Sprint B.3 — Workflow tab data hook                                 */
/* ------------------------------------------------------------------ */

import type { WorkflowStepWithMeta, StepOverride } from "@/lib/workflow-merge";

export type WorkflowDiagramResponse = {
  property_id: string;
  compiled_at: string;
  source_settings_hash: string;
  steps: WorkflowStepWithMeta[];
  overrides: Record<string, StepOverride>;
  has_overrides: boolean;
};

export function useWorkflow(propertyId: string | null | undefined) {
  const [diagram, setDiagram] = useState<WorkflowDiagramResponse | null>(null);
  const [loading, setLoading] = useState(true);
  ...
  const refetch = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/property/${propertyId}/workflow`);
      if (res.status === 404) {
        // Not compiled yet — leave diagram null and surface a "not compiled"
        // signal to the UI via error.
        const body = await res.json().catch(() => ({}));
        setError(body.error || "Workflow not yet compiled for this property");
        setDiagram(null);
      } else if (!res.ok) {
        const body = await res.json().catch(() => ({}));
        setError(body.error || `Workflow fetch failed (${res.status})`);
        setDiagram(null);
      } else {
        const data = (await res.json()) as WorkflowDiagramResponse;
        setDiagram(data);
      }
    } catch (e) {
      setError(String(e));
  ...
  const recompile = useCallback(async () => {
    ...
    setLoading(true);
    try {
      await fetch(`/api/property/${propertyId}/recompile-workflow`, {
        method: "POST",
      });
    } catch (e) {
  ...
  const saveOverride = useCallback(
    async (stepId: string, override: StepOverride) => {
      if (!propertyId) return;
      const res = await fetch(`/api/property/${propertyId}/workflow`, {
        method: "PATCH",
      ...
      if (!res.ok) {
        const body = await res.json().catch(() => ({}));
        throw new Error(body.error || `Save override failed (${res.status})`);
      }
      await refetch();
    },
    [propertyId, refetch]
  ...
  const clearOverride = useCallback(
    async (stepId: string) => {
      ...
      const res = await fetch(`/api/property/${propertyId}/workflow`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ step_id: stepId, override: null }),
      });
      if (!res.ok) {
        const body = await res.json().catch(() => ({}));
        throw new Error(body.error || `Clear override failed (${res.status})`);
      }
      await refetch();
    },
    [propertyId, refetch]
